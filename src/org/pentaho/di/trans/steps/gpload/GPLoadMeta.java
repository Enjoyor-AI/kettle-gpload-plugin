...
public class GPLoadMeta extends BaseStepMeta implements StepMetaInterface {
	...
	/**
	 * NULL_AS parameter for gpload - gpload treats values matching this string as
	 * null
	 */
	...
	public static final String[] SUPPORTED_ENCODINGS = { "", "BIG5", "EUC_CN", "EUC_JP", "EUC_KR", "EUC_TW", "GB18030",
			"GBK", "ISO-8859-1", "ISO_8859_5", "ISO_8859_6", "ISO_8859_7", "ISO_8859_8", "JOHAB", "KOI8", "LATIN1",
			"LATIN2", "LATIN3", "LATIN4", "LATIN5", "LATIN6", "LATIN7", "LATIN8", "LATIN9", "LATIN10", "MULE_INTERNAL",
			"SJIS", "SQL_ASCII", "UHC", "UTF8", "WIN866", "WIN874", "WIN1250", "WIN1251", "WIN1252", "WIN1253",
	...
	private void readData(Node stepnode, List<? extends SharedObjectInterface> databases) throws KettleXMLException {
		try {
			...
			throw new KettleXMLException(
					BaseMessages.getString(PKG, "GPLoadMeta.Exception.UnableToReadStepInfoFromXML"), e);
		}
	}
	...
	public void readRep(Repository rep, IMetaStore metaStore, ObjectId id_step, List<DatabaseMeta> databases)
		...
		try {
			...
			throw new KettleException(
					BaseMessages.getString(PKG, "GPLoadMeta.Exception.UnexpectedErrorReadingStepInfoFromRepository"),
					e);
		}
	}
		...
		if (databaseMeta != null) {
			...
			try {
				...
				if (!Const.isEmpty(tableName)) {
					...
					if (r != null) {
						...
						if (error_found) {
							...
							cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK,
									BaseMessages.getString(PKG, "GPLoadMeta.CheckResult.AllFieldsFoundInTargetTable"),
									stepMeta);
						}
						remarks.add(cr);
					} else {
				...
				if (prev != null && prev.size() > 0) {
					cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK,
							BaseMessages.getString(PKG, "GPLoadMeta.CheckResult.StepReceivingDatas", prev.size() + ""),
							stepMeta);
					remarks.add(cr);
					...
					for (int i = 0; i < fieldStream.length; i++) {
						...
						if (v == null) {
							if (first) {
								first = false;
								error_message += BaseMessages.getString(PKG,
										"GPLoadMeta.CheckResult.MissingFieldsInInput") + Const.CR;
							}
							error_found = true;
					...
					error_message = BaseMessages.getString(PKG, "GPLoadMeta.CheckResult.MissingFieldsInInput3")
							+ Const.CR;
					cr = new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR, error_message, stepMeta);
					remarks.add(cr);
				}
			} catch (KettleException e) {
				error_message = BaseMessages.getString(PKG, "GPLoadMeta.CheckResult.DatabaseErrorOccurred")
						+ e.getMessage();
				cr = new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR, error_message, stepMeta);
				remarks.add(cr);
			} finally {
			...
			if (prev != null && prev.size() > 0) {
				...
				if (!Const.isEmpty(tableName)) {
					...
					try {
						...
						retval.setError(
								BaseMessages.getString(PKG, "GPLoadMeta.GetSQL.ErrorOccurred") + e.getMessage());
					}
				} else {
		...
		if (prev != null) {
			...
			for (int i = 0; i < fieldTable.length; i++) {
				...
				DatabaseImpact ii = new DatabaseImpact(DatabaseImpact.TYPE_IMPACT_READ_WRITE, transMeta.getName(),
						stepMeta.getName(), databaseMeta.getDatabaseName(), transMeta.environmentSubstitute(tableName),
						fieldTable[i], fieldStream[i], v != null ? v.getOrigin() : "?", "",
						"Type = " + v.toStringMeta());
				impact.add(ii);
			}
		}
